...
*/
    ...
    /* Returns false if the current solution would be impossible to finalize into a working solution
    ...
    */
    public boolean stillPossible(boolean nextValue, GameModel model) {
		...
		if ((i > 0) && ((model.isON(i - 1, j) && oddNeighborhood(i - 1, j))||(!model.isON(i - 1, j) && !oddNeighborhood(i - 1, j)))) {
				possible = false;
		} 
		if (possible && i == (height - 1)) {
			if ((j > 0)&& ((model.isON(i, j - 1) && oddNeighborhood(i, j - 1))||(!model.isON(i, j - 1) && !oddNeighborhood(i, j - 1)))) {
				possible = false;

			}if (possible && (j == (width - 1)) && ((model.isON(i, j) && oddNeighborhood(i, j))||(!model.isON(i, j) && !oddNeighborhood(i, j)))) {
				possible = false;
			}
		}
		board[i][j] = before;
    ...
    /**
     * this method attempts to finish the board of GameModel model. 
     * this method assumes that the solution is currently still extendable for a board in the 
    ...
    // returns true if the position at column i, row j is tapped in the solution, false otherwise
